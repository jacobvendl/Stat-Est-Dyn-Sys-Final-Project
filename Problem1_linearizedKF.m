...
for t=1:length(T)
    ...
    [F, Omega] = F_G_variant(dx(1,t),dx(3,t));
...
for k=1:length(T)
    ...
    [F, Gamma] = F_G_variant(dx_hat_plus(1,k),dx_hat_plus(3,k));
    dx_hat_minus(:,k+1) = F*dx_hat_plus(:,k) + Gamma * wk;
    P_minus = F*P_plus*F' + Gamma*Q*Gamma';
    
    H = H_variant(X(k+1),XD(k+1),Y(k+1),YD(k+1),Xs(k+1),XDs(k+1),Ys(k+1),YDs(k+1));
    K = P_minus*H'*inv(H*P_minus*P' + R);
    P_plus = (eye(4) - K*H)*P_minus;
    
    %loop through the stations to establish sensor measurement at k
    for i=1:12
        if ~isnan(rho_per(i,t))
            %find y_star at the given time
            
            
        end
    end
    
    dx_hat_plus(:,k+1) = dx_hat_minus(:,k+1) + K*(dy - H*dx_hat_minus(:,k+1));
...
function [F Omega] = F_G_variant(X,Y)
mu = 398600;        % km^3/s^2
r0_nom = 6678;          % km
dt = 10;
